...
function App() {
  ...
  useEffect(() => {
    if (jwtToken === "") {
      ...
      fetch(`/refresh`, requestOptions)
        ...
        .catch(error => {
          console.log("user is not log in", error);
        })
    }
  }, [jwtToken])
  ...
  return (
    <div className="container">
      <div className="row">
        <div className="col">
          <h1 className="mt-3"> Movie List </h1>
        </div>
        <div className="col text-end">
          {jwtToken === ""
            ? <Link to="/login">
              <span className="badge bg-success"> Login </span></Link>
            : <a href="#!" onClick={logOut}>
              <span className="badge bg-danger"> Logout </span></a>
          }
        </div>
